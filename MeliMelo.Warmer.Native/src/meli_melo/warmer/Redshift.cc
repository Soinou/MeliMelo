 ...
 *
 * Content was modified a bit, and since I'm not quite sure how the licenses work, I don't know
 * exactly what I am supposed to do. If there is some kind of problem with this, then please feel
 * free to tell me so.
 */
...
namespace MeliMelo
{
    namespace Warmer
    {
        namespace Redshift
        {
            ...
            const int Uint16Max = UINT16_MAX + 1;
            ...
            static uint16_t get_value(double* white_point, const uint16_t& gamma, unsigned short c)
            {
                return static_cast<uint16_t>(pow((static_cast<double>(gamma) / Uint16Max)
                                                 * white_point[c], 1) * Uint16Max);
            }
                                 ...
                                 int temperature)
            {
                ...
                {
                    gamma_r[i] = get_value(white_point, gamma_r[i], 0);
                    gamma_g[i] = get_value(white_point, gamma_g[i], 1);
                    gamma_b[i] = get_value(white_point, gamma_b[i], 2);
                }
            }
        }
    }
